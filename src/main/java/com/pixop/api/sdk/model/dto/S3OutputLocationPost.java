...
public class S3OutputLocationPost {
   ...
   * @throws IOException if the JSON Element is invalid with respect to S3OutputLocationPost
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!S3OutputLocationPost.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in S3OutputLocationPost is not found in the empty JSON string", S3OutputLocationPost.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!S3OutputLocationPost.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `S3OutputLocationPost` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : S3OutputLocationPost.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!S3OutputLocationPost.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'S3OutputLocationPost' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<S3OutputLocationPost> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(S3OutputLocationPost.class));

       return (TypeAdapter<T>) new TypeAdapter<S3OutputLocationPost>() {
           @Override
           public void write(JsonWriter out, S3OutputLocationPost value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public S3OutputLocationPost read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of S3OutputLocationPost given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of S3OutputLocationPost
   * @throws IOException if the JSON string is invalid with respect to S3OutputLocationPost
   */
  public static S3OutputLocationPost fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, S3OutputLocationPost.class);
  }

  /**
   * Convert an instance of S3OutputLocationPost to an JSON string
   *
   * @return JSON string
   */
