...
public class VideoProcessingState {
   ...
   * @throws IOException if the JSON Element is invalid with respect to VideoProcessingState
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VideoProcessingState.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in VideoProcessingState is not found in the empty JSON string", VideoProcessingState.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VideoProcessingState.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VideoProcessingState` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("processingStatus") != null && !jsonObj.get("processingStatus").isJsonNull()) {
        OperationStatus.validateJsonElement(jsonObj.get("processingStatus"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!VideoProcessingState.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'VideoProcessingState' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<VideoProcessingState> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(VideoProcessingState.class));

       return (TypeAdapter<T>) new TypeAdapter<VideoProcessingState>() {
           @Override
           public void write(JsonWriter out, VideoProcessingState value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public VideoProcessingState read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of VideoProcessingState given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of VideoProcessingState
   * @throws IOException if the JSON string is invalid with respect to VideoProcessingState
   */
  public static VideoProcessingState fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, VideoProcessingState.class);
  }

  /**
   * Convert an instance of VideoProcessingState to an JSON string
   *
   * @return JSON string
   */
