...
public class VideoProcessingConfigurationApi extends BaseApi {
   ...
   * Creates a new video processing configuration for the selected team.  **Important:**   The &#x60;options&#x60; field is required when creating a new configuration.   Due to a limitation in our API documentation provider, &#x60;options&#x60; is not marked as required in the OpenAPI schema to prevent the interactive request builder from pre-populating unintended fields. Requests submitted without &#x60;options&#x60; will be rejected by the API.  **Using the interactive request builder:**   Some optional nested settings (such as certain filter and color-space options) may remain present in the request after being selected once, even if they are later cleared in the UI. If you encounter validation errors or unexpected behavior, please review the generated request payload before submitting it. Refreshing the page will reset the request builder and remove any previously selected options. 
